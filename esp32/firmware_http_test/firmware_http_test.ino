/*
  firmware_http_test.ino  —  Test de integración completo con OLED
...
#define I2S_WS_PIN      1   // igual que firmware_http.ino — hardware wired aquí
#define I2S_SD_PIN      3
...
const char* SERVER_URL = "https://crzs1qs4-5321.use.devtunnels.ms/audio/pcm16";
...
// ── Grabación (mismos valores que firmware_http.ino) ──────────────────────
#define REC_SAMPLE_RATE    8000
#define REC_DURATION_S     5
#define REC_AUDIO_BYTES    ((size_t)REC_DURATION_S * REC_SAMPLE_RATE * 2)
#define CHUNK_SIZE         4096   // bytes 32-bit por lectura I2S
#define GAIN_SHIFT         14     // confirmado: >>14 da rango ±8000-30000
#define WARMUP_DURATION_MS 100    // 100 ms para convergencia del EMA de DC
...
// Eventos publicados cada ciclo — solo procesarTouch() los escribe
bool ev_shortTap   = false;
bool ev_longHold   = false;   // dispara UNA VEZ al soltar después de hold
bool ev_tapsReady  = false;   // ventana de taps expiró
int  ev_tapCount   = 0;
bool ev_isHolding  = false;   // true mientras sigue presionado ≥ HOLD_MS

// ── Modos y estado de pantalla ────────────────────────────────────────────
#define NUM_MODES 3   // 0=test-HTTP  1=…  2=…  (expandir luego)
int appMode = 0;
...
// Dibuja ojos + etiqueta en la franja inferior
...
// Pantalla de texto simple (sin ojos) para estados intermedios
...
// ═══════════════════════════════════════════════════════════════════════════
//  I2S — 8 kHz / 32-bit (igual que firmware_http.ino)
// ═══════════════════════════════════════════════════════════════════════════
...
// Copiada exacta de firmware_http.ino — no modificar
size_t convert_i2s32_to_pcm16(const uint8_t* input, size_t input_len, uint8_t* output) {
  ...
  static int64_t dc_ema = 0;  // EMA DC removal — alpha=1/64, τ≈8ms @8kHz
  double sum_sq = 0.0;
  int32_t max_abs = 0;
...
// ═══════════════════════════════════════════════════════════════════════════
//  TOUCH — lectura única por ciclo
//
//  Comportamiento:
//    mantener (≥ HOLD_MS)   → ev_isHolding=true mientras dura
//                              ev_longHold=true al soltar  → mic
//    3 taps cortos           → ev_tapsReady + ev_tapCount=3 → modo siguiente
// ═══════════════════════════════════════════════════════════════════════════
...
void procesarTouch() {
  ...
  // Mientras se mantiene presionado ≥ HOLD_MS
...
void setup(){
  ...
  appState = ST_LISTO;
  setMood(MOOD_NORMAL);
  Serial.println("Listo. Toca para grabar.");
}
...
void loop(){
  ...
  // ── HOLD → micrófono (sin importar modo) ────────────────────────────────
  ...
  // ── Acumular audio durante grabación (lógica idéntica a firmware_http.ino) ──
  if (recRecording) {
    ...
    if (r == ESP_OK && bytesRead > 0) {
      ...
      if (pcm16_len > 0) {
        // Warmup: descartar primeros 100 ms mientras EMA de DC converge
        if (!warmup_complete) {
          ...
          // chunk descartado — continúa al render
        } else {
          ...
          _rec_len += to_copy;
          if (_rec_len >= REC_AUDIO_BYTES) stopAndSend();  // auto-stop 5 s
        }
      }
    }
  }
  ...
  // ── Volver a LISTO después de 4 s mostrando resultado ──────────────────
  ...
  switch(appState){
    ...
    case ST_GRABANDO: {
      ...
      drawEye(leftEye, true);
      drawEye(rightEye, false);
      // Barra de nivel en la franja inferior
      ...
      // Sobreescribir si la barra es muy larga — sólo mostrar cuando hay espacio
    ...
    case ST_RESULTADO: {
      // Ojos con el mood recibido + intent
      char label[24];
